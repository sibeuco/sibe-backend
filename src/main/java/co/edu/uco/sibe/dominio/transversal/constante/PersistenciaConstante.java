...
public final class PersistenciaConstante {
    ...
    public static final String CONSULTAR_FECHAS_REALIZACION_POR_ESTADO = "SELECT ea.fechaRealizacion FROM EjecucionActividadEntidad ea WHERE ea.estadoActividad.estado.nombre = :estado AND ea.fechaRealizacion IS NOT NULL ORDER BY ea.fechaRealizacion DESC";
    public static final String CONSULTAR_SEMESTRES_ESTUDIANTES_FINALIZADOS = "SELECT DISTINCT pe.semestreActual FROM RegistroAsistenciaEntidad ra JOIN ra.ejecucionActividad ea JOIN ParticipanteEstudianteEntidad pe ON pe.identificador = ra.participante.identificador WHERE ea.estadoActividad.estado.nombre = :estado AND pe.semestreActual IS NOT NULL ORDER BY pe.semestreActual DESC";
    public static final String CONSULTAR_CENTROS_COSTOS_EMPLEADOS_FINALIZADOS = "SELECT DISTINCT cc.descripcion FROM RegistroAsistenciaEntidad ra JOIN ra.ejecucionActividad ea JOIN ParticipanteEmpleadoEntidad pe ON pe.identificador = ra.participante.identificador JOIN pe.centroCostos ecc JOIN ecc.centroCostos cc WHERE ea.estadoActividad.estado.nombre = :estado ORDER BY cc.descripcion ASC";
    public static final String CONSULTAR_TIPOS_PARTICIPANTES_FINALIZADOS = "SELECT DISTINCT CASE WHEN TYPE(p) = ParticipanteEstudianteEntidad THEN 'ESTUDIANTE' WHEN TYPE(p) = ParticipanteEmpleadoEntidad THEN 'EMPLEADO' WHEN TYPE(p) = ParticipanteExternoEntidad THEN 'EXTERNO' END FROM RegistroAsistenciaEntidad ra JOIN ra.ejecucionActividad ea JOIN ra.participante p WHERE ea.estadoActividad.estado.nombre = :estado ORDER BY 1 ASC";
    public static final String CONSULTAR_PROGRAMAS_ESTUDIANTES_FINALIZADOS = "SELECT DISTINCT pe.programaAcademico FROM RegistroAsistenciaEntidad ra JOIN ra.ejecucionActividad ea JOIN ParticipanteEstudianteEntidad pe ON pe.identificador = ra.participante.identificador WHERE ea.estadoActividad.estado.nombre = :estado ORDER BY pe.programaAcademico ASC";
    public static final String CONSULTAR_INDICADORES_EJECUCIONES_FINALIZADAS = "SELECT DISTINCT i.nombre FROM EjecucionActividadEntidad ea JOIN ea.actividad a JOIN a.indicador i WHERE ea.estadoActividad.estado.nombre = :estado ORDER BY i.nombre ASC";
    ...
    public static final String ESTADO_PARAMETRO = "estado";
